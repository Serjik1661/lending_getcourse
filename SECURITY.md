...
- Service worker кэширует только локальные статические файлы и offline/404 fallback; внешние домены, Яндекс.Карты, мессенджеры и GetCourse не кэшируются.
...
- При изменении файлов из списка `LOCAL_ASSETS` в `sw.js` увеличить `CACHE_NAME`.
